function [Vent_Fold,Vent_Anat_Fold,Diff_Fold] = find_dicoms(participant_folder,copyfiles)
...
for i = 1:length(dcm_indx)
    ...
    try
        Desc{i,1} = tmpinfo.SeriesDescription;
    catch
        Desc{i,1} = 'none';
    end      
end
...
try
    Vent_GRE_ind = (contains(ProtName,'vent','IgnoreCase',true) | contains(ProtName,'gre_hpg') | contains(Desc,'2D FGRE')) & contains(Nucleus,'Xe','IgnoreCase',true);
    
    last_vent_series = max(series_no(Vent_GRE_ind));
    
    Vent_target_ind = (contains(ProtName,'vent','IgnoreCase',true) | contains(ProtName,'gre_hpg') | contains(Desc,'2D FGRE')) & contains(Nucleus,'Xe','IgnoreCase',true) & series_no == last_vent_series;
...
try
    Anat_GRE_ind = (contains(ProtName,'vent','IgnoreCase',true) |  contains(Desc,'2D FSE') |  contains(Desc,'mask')) & contains(Nucleus,'H','IgnoreCase',true);
    
    last_anat_series = max(series_no(Anat_GRE_ind));
    
    Anat_target_ind = (contains(ProtName,'vent','IgnoreCase',true) |  contains(Desc,'2D FSE') |  contains(Desc,'mask')) & contains(Nucleus,'H','IgnoreCase',true) & series_no == last_anat_series;
...
try
    Diff_ind = (contains(ProtName,'Diff','IgnoreCase',true) |  contains(Desc,'2D Diffusion')) & contains(Nucleus,'Xe','IgnoreCase',true);
    
    last_diff_series = max(series_no(Diff_ind));
    
    diff_target_ind = (contains(ProtName,'Diff','IgnoreCase',true) |  contains(Desc,'2D Diffusion')) & contains(Nucleus,'Xe','IgnoreCase',true) & series_no == last_diff_series;
