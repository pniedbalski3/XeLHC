function [Membrane, RBC] = dixon_sep(DissolvedImage,SpecRBCBarrierRatio,GasImage,LungMask)
...
Membrane = imag(DixonImage);
RBC = real(DixonImage);

if(mean(Membrane(LungMask),'all') < 0)
    Membrane = -imag(DixonImage);
end

if(mean(RBC(LungMask),'all') < 0)
    RBC = -real(DixonImage);
end
